...
export const User = () => {
  ...
  useEffect(() => {
    fetch('http://192.168.1.6:3000/api/v1/customers')
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/api/v1/customers');
      if (response.ok) {
  ...
  const handleupdate = customer => {
    fetch(`http://192.168.1.6:3000/api/v1/customers/${customer.id}`, {
      method: 'PUT',
  ...
  const handleDelete = async id => {
    ...
    if (confirmed) {
      const response = await fetch(`http://192.168.1.6:3000/api/v1/customers/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
