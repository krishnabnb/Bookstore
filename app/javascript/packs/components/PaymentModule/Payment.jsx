...
export const Payment = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch('http://192.168.1.8:3000/api/v1/payments');
      if (response.ok) {
  ...
  const handleUpdate = payment => {
    fetch(`http://192.168.1.8:3000/api/v1/payments/${payment.id}`, {
      method: 'PUT',
  ...
  const handleDelete = async id => {
    ...
    if (confirmed) {
      try {
        await fetch(`http://192.168.1.8:3000/api/v1/payments/${id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json'
          }
