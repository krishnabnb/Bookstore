...
export const Payment = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/api/v1/payments');
      if (response.ok) {
  ...
  const fetchCarts = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/api/v1/carts');
      if (response.ok) {
        const data = await response.json();
        setCarts(data);
      } else {
  ...
  const handleFormSubmit = (mathod, cart_id) => {
    ...
    fetch('http://192.168.1.6:3000/api/v1/payments', {
      method: 'POST',
      headers: {
  ...
  const handleUpdate = payment => {
    fetch(`http://192.168.1.6:3000/api/v1/payments/${payment.id}`, {
      method: 'PUT',
  ...
  const handleDelete = async id => {
    ...
    if (confirmed) {
      try {
        await fetch(`http://192.168.1.6:3000/api/v1/payments/${id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json'
          }
