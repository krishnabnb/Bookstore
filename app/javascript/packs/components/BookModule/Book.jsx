...
export const Book = () => {
  ...
  const fetchBookDetails = async (id) => {
    try {
      const response = await fetch(`http://192.168.1.6:3000/api/v1/books/${id}`);
      if (!response.ok) {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/api/v1/books');
      if (!response.ok) {
  ...
  const handleUpdate = async book => {
    fetch(`http://192.168.1.6:3000/api/v1/books/${book.id}`, {
      method: 'PUT',
  ...
  const handleImageChange = async (e, book) => {
    try {
      ...
      const response = await fetch(`http://192.168.1.6:3000/api/v1/books/${book.id}`, {
        method: 'PUT',
        body: formdata,
      });
  ...
  const handleToggleStatus = async (id) => {
    try {
      const response = await fetch(`http://192.168.1.6:3000/api/v1/books/${id}/update_status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleSearch = async () => {
    try {
      ...
      const url = `http://192.168.1.6:3000/api/v1/books?${queryParams.toString()}`;
  ...
  const handleImageDelete = async (bookId, type) => {
    try {
      const response = await fetch(`http://192.168.1.6:3000/api/v1/books/${bookId}/image_destroy`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
